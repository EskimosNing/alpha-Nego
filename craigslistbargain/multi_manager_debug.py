...
class MultiRunner:
        ...
        if system.env.name == 'pt-neural-dsac':
            ...
            if system.env.tom_model is not None:
                ...
                if args.tom_model not in ['naive', 'history']:
                    print("tom.encoder:", system.env.tom_model.encoder.identity)
    ...
    def valid(self, start, length):
        infos = self.trainer.validate(self.args, length,
                                      start=start)  # [total_stats, oppo_total_stats], examples, verbose_str
        return infos
    ...
    def train_dsac(self, batches, update_policy, update_critic):
        ...
        if update_policy is True and update_critic is True:
            ...
            for i in range(10):
                zf1_loss, zf2_loss, _, _, _, _, _ = self.trainer.train_from_torch(batches, update_policy=False,
                                                                                  update_critic=True, is_pretrain=False)
            for i in range(5):
                _, _, policy_loss, alpha_intent, alpha_price, q_loss, kl_loss = self.trainer.train_from_torch(batches,
                                                                                                              update_policy=True,
                                                                                                              update_critic=False,
                                                                                                              is_pretrain=False)
            ...
            for i in range(10):
                zf1_loss, zf2_loss, _, _, _, _, _ = self.trainer.train_from_torch(batches, update_policy=False,
                                                                                  update_critic=True, is_pretrain=True)
...
class MultiManager():
            ...
            if args.only_run:
                ...
                for j in range(len(worker.trainer.hidden_vec)):
                    assert worker.trainer.hidden_vec[j].shape[0] == worker.trainer.hidden_stra[j].shape[0], \
                        "miss match at {}, {} of {}".format(worker.trainer.hidden_vec[j].shape,
                                                            worker.trainer.hidden_stra[j].shape, j)
    ...
    def learn_dsac(self):
        ...
        for epoch in range(max_epoch):
            ...
            if (epoch + 1) % 10 == 0 and not self.args.self_play:
                ...
                self.writer.add_scalar('Evaluate agent dsac of  success rate std', result.get('success_rate_std'),
                                       epoch)
            ...
            if epoch > 100 and (epoch+1) % 10 == 0 and self.args.self_play:
                ...
                # ['insist', 'decay', 'persuaded', 'convex', 'concave', 'low', 'high', 'behavior_kind', 'behavior_unkind']
                ...
                for kind in self.trainer.trainer.opponents_pool.keys():
                    for opponent in self.trainer.trainer.opponents_pool[kind].keys():
                        ...
                        infos = worker.local_send(
                            ['valid_sp', (0, 20, self.trainer.trainer.opponents_pool[kind][opponent])])
                        examples = infos[1][0]
                ...
                if self.check_total_performance():
                    ...
                    self.trainer.trainer.opponents_pool['rl'][
                        "rl_" + str(len(self.trainer.trainer.opponents_pool['rl']))] = \
                        DSACNeuralSystem(args, schema, lexicon, args.agent_checkpoints[0], False, name='pt-neural-dsac',
                                         id=id, dsac_load_dict=dsac_load_dict)
                    self.trainer.trainer.score_table['rl']["rl_" + str(len(self.trainer.trainer.score_table['rl']))] = {
                        'success_rate': [0.5], 'utility': [0.5], 'length': [10], 'fairness': [1], 'score': [1.46],
                        'win_rate': [0.5]}
    ...
    def check_each_performance(self, opponent_kind):
        ...
        print("self.trainer.trainer.score_table[opponent_kind].keys()",
              self.trainer.trainer.score_table[opponent_kind].keys())
    ...
    def save_dsac_model(self, epoch, args):
        torch.save(self.trainer.trainer.policy.state_dict(),
                   args.model_path + "/actor_seed" + str(args.seed) + "_" + str(epoch + 1) + ".pth")
        torch.save(self.trainer.trainer.zf1.state_dict(),
                   args.model_path + "/zf1_seed" + str(args.seed) + "_" + str(epoch + 1) + ".pth")
        torch.save(self.trainer.trainer.zf1.state_dict(),
                   args.model_path + "/zf2_seed" + str(args.seed) + "_" + str(epoch + 1) + ".pth")
    ...
    def learn(self):
        ...
        for epoch in range(max_epoch):
            ...
            print('\t<train> reward{:.3f}, {:.3f} pg_loss {:.5f}, value_loss {:.5f}, value_update {}'
                  .format(np.mean(_rewards[0]), np.mean(_rewards[1]), loss['pg_loss'][0, 0], loss['value_loss'][0, 0],
                          value_update))
    ...
    def add_to_replay_buffer(self, batch_iters, rewards, replay_buffer):
        for idx, batch_iter in enumerate(batch_iters[0]):
            for i in range(len(batch_iter)):
                transition = []
                transition.extend(
                    [batch_iter[i].state_0.squeeze(), batch_iter[i].state_1.squeeze(), batch_iter[i].uttr[0].squeeze()])
                transition.extend(
                    [batch_iter[i].act_0.squeeze(), batch_iter[i].act_2.squeeze(), batch_iter[i].act_3.squeeze()])
                if i != len(batch_iter) - 1:
                    transition.extend([batch_iter[i + 1].state_0.squeeze(), batch_iter[i + 1].state_1.squeeze(),
                                       batch_iter[i + 1].uttr[0].squeeze()])
                    if batch_iter[i + 1].act_0[0][0].item() == batch_iter[i + 1].acc_idx or batch_iter[i + 1].act_0[0][
                        0].item() == batch_iter[i + 1].rej_idx:
                    ...
                    if batch_iter[i].act_0[0][0].item() == batch_iter[i].offer_idx:
                        transition.extend([batch_iter[i].state_0.squeeze(), batch_iter[i].state_1.squeeze(),
                                           batch_iter[i].uttr[0].squeeze()])
                        transition.append(rewards[0][idx])  # reward
                        transition.append(1.0)  # done bool
                    else:
                        continue
    ...
    def run(self):
        ...
        for epoch in range(args.start_epoch, max_epoch):
            ...
            # Train the model
            train_info = self.worker_conn[0].send(
                ['train', pkl.dumps((epoch, batches, rewards[0], self.args.train_mode))])
