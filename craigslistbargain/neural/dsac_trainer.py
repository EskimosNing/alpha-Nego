...
class DSACTrainer(BaseTrainer):
    def __init__(self, agents, scenarios, train_loss, optim, training_agent=0, reward_func='margin',
        ...
        self._price_actions_masks = torch.sum(
            F.one_hot(torch.LongTensor(self.price_action_indexs), num_classes=self.dsac.intent_size), dim=0).float()
        ...
        if args.self_play:
            ...
            for i in self.rule_strategies:
                ...
                self.score_table['sl'][i] = {'success_rate': [0.0], 'utility': [0.0], 'length': [10], 'fairness': [0],
                                             'score': [1], 'win_rate': [0]}
            self.score_table['self']['self_agent'] = {'success_rate': [0.0], 'utility': [0.0], 'length': [10],
                                                      'fairness': [0], 'score': [1], 'win_rate': [0]}
    ...
    def update_score_table(self, info):
        ...
        if opponent_type.get(info['opponent_name']) is None:
            opponent_type[info['opponent_name']] = {'success_rate': [0.0], 'utility': [0.0], 'length': [10],
                                                    'fairness': [0.0], 'score': [1.0], 'win_rate': [0.0]}
        ...
        item['score'].append(
            self.calculate_score(info['success_rate'], info['utility'], info['length'], info['fairness']))
    ...
    def get_opponent_system(self):
        ...
        if sample_kind in ['sl', 'rl']:
            ...
            prob = [self.score_table[sample_kind][item]['win_rate'][-1] for item in
                    self.score_table[sample_kind].keys()]
    ...
    def example_to_text(self, example):
        ...
        for i, e in enumerate(example.events):
            if "real_uttr" in e.metadata.keys():
                ret.append(
                    "[{}: {}]\t{}\t{}\t\"{}\"".format(e.time, e.agent, e.action, e.data, e.metadata["real_uttr"]))
            else:
    ...
    def get_eval_dict_sp(self, examples, opponent):
        ...
        # 'success_rate': [0.0], 'utility': [0.0], 'length': [10],
        #                                               'fairness': [0], 'score': [1], 'win_rate': [0]
        if 'success_rate' in  eval_dict:
            avg_success_rate = np.mean(eval_dict['success_rate'])
        if 'utility' in  eval_dict:
            avg_utility = np.mean(eval_dict['utility'])
        if 'length' in  eval_dict:
            avg_length = np.mean(eval_dict['length'])
        if 'fairness' in  eval_dict:
        ...
        info = {'success_rate': avg_success_rate, 'utility': avg_utility, 'length': avg_length, \
                'fairness': avg_fairness, 'opponent_name': opponent, 'opponent_type': self.get_opponent_kind(opponent),
                'win_rate': win_rate}
    ...
    def get_avg_utility(self):
        ...
        for kind in self.score_table.keys():
            for opponent in self.score_table[kind].keys():
                res.append(self.score_table[kind][opponent]['utility'][-1])
        return np.mean(res)
    ...
    def get_avg_success_rate(self):
        ...
        for kind in self.score_table.keys():
            for opponent in self.score_table[kind].keys():
                res.append(self.score_table[kind][opponent]['success_rate'][-1])
        return np.mean(res)

    def get_avg_fairness(self):
        res = []
        for kind in self.score_table.keys():
            for opponent in self.score_table[kind].keys():
                res.append(self.score_table[kind][opponent]['fairness'][-1])
        return np.mean(res)

    def get_avg_win_rate(self):
        res = []
        for kind in self.score_table.keys():
            for opponent in self.score_table[kind].keys():
                res.append(self.score_table[kind][opponent]['win_rate'][-1])
        return np.mean(res)
    ...
    def get_avg_length(self):
        ...
        for kind in self.score_table.keys():
            for opponent in self.score_table[kind].keys():
                res.append(self.score_table[kind][opponent]['length'][-1])
        return np.mean(res)
    ...
    def get_action_full_information(self, model, batch_uttr, batch_state):
        ...
        last_intents_one_hot = batch_state[:,
                               (state_length - 1) * self.dsac.intent_size: state_length * self.dsac.intent_size]
        ...
        return intent_one_hot, price_one_hot, intent_log_probability.max(dim=1)[0].unsqueeze(dim=1), \
               price_log_probability.max(dim=1)[0].unsqueeze(dim=1), intent_gumbel_softmax, sl_intent_softmax
    ...
    def train_from_torch(self, batch, update_policy=True, update_critic=True, is_pretrain=False):
        ...
        if update_critic:
            with torch.no_grad():
                ...
                new_next_intents_onehot, new_next_prices_onehot, new_next_intents_logpi, new_next_prices_logpi, _, _ = self.get_action_full_information(
                    self.target_policy, batch_next_uttr, batch_next_state)
                ...
                if is_pretrain:
                    target_z_values = torch.min(target_z1_values,
                                                target_z2_values)  # - alpha * new_next_intents_logpi - 0.2 * alpha * new_next_prices_logpi
                else:
                    target_z_values = torch.min(target_z1_values,
                                                target_z2_values) - 0.1 * alpha * new_next_intents_logpi - alpha * new_next_prices_logpi
        ...
        if update_policy:
            ...
            # print("kl", 0.01 * kl[0])
            policy_loss = (
                        0.1 * alpha * new_intents_log_probabilities + alpha * new_prices_log_probabilities - q_new_actions + self.args.kl_coefficient * kl).mean()
        ...
        if update_policy:
            return zf1_loss, zf2_loss, policy_loss, (0.1 * alpha * new_intents_log_probabilities).mean(), (
                        alpha * new_prices_log_probabilities).mean(), \
